...
async function carregar_produtos() {
    ...
    itens.forEach(p => {
        ...
        div.className = 'item'
        div.innerHTML = `<div><b>${p.codigo}</b> ${p.nome} - R$ ${p.valor.toFixed(2)}</div>
      <div>
...
async function carregar_materias() {
    ...
    itens.forEach(m => {
        const div = document.createElement('div')
        div.className = 'item'
        div.innerHTML = `<div><b>${m.codigo}</b> ${m.nome} - estoque: ${m.quantidade_estoque}</div>
      <div>
...
async function carregar_ingredientes() {
    ...
    itens.forEach(a => {
        const div = document.createElement('div')
        div.className = 'item'
        div.innerHTML = `<div>#${a.id} - Matéria ${a.materia_prima_id} precisa ${a.quantidade_necessaria}</div>
      <div>
...
async function salvar_materia(e) {
    ...
    const quantidade_estoque = parseInt(document.getElementById('materia-estoque').value)
    if (!codigo || !nome || isNaN(quantidade_estoque)) return
    await fetch(api.materias, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ codigo, nome, quantidade_estoque })
    })
    e.target.reset()
    carregar_materias()
}
...
async function adicionar_ingrediente() {
    ...
    const quantidade_necessaria = parseInt(document.getElementById('quantidade-necessaria').value)
    if (!produto_id || !materia_prima_id || !quantidade_necessaria) return
    await fetch(`${api.associacoes}/produto/${produto_id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ materia_prima_id, quantidade_necessaria })
    })
...
async function editar_excluir_lista(e) {
    ...
    if (acao === 'excluir-ingrediente') {
        ...
        const quantidade_necessaria = prompt('Nova quantidade necessária:')
        if (!quantidade_necessaria) return
        await fetch(`${api.associacoes}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ materia_prima_id: 0, quantidade_necessaria: parseInt(quantidade_necessaria) }) })
        carregar_ingredientes()
    }
}
...
async function editar_excluir_materias(e) {
    ...
    if (acao === 'excluir') {
        ...
        const nome = prompt('Novo nome:')
        const quantidade_estoque = prompt('Nova quantidade em estoque:')
        if (!nome && !quantidade_estoque) return
        const body = {}
        if (nome) body.nome = nome
        if (quantidade_estoque) body.quantidade_estoque = parseInt(quantidade_estoque)
...
async function calcular_producao() {
    ...
    dados.itens.forEach(i => {
        const div = document.createElement('div')
        div.className = 'item'
        div.innerHTML = `<div><b>${i.codigo}</b> ${i.nome} → ${i.quantidade} un</div><div>R$ ${i.valor_total_item.toFixed(2)}</div>`
        lista.appendChild(div)
    })
    total.textContent = `Valor total: R$ ${dados.valor_total.toFixed(2)}`
    carregar_materias()
}
